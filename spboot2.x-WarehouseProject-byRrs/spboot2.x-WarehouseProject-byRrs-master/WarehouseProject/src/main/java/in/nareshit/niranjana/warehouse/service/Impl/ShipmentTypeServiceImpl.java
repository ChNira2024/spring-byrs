...
import in.nareshit.niranjana.warehouse.repo.ShipmentTypeRepository;
...
implements IShipmentTypeService
{
	...
	public ShipmentType getShipmentType(Integer id) 
	{
		return repo.findById(id).orElseThrow(()->new ShipmentTypeNotFoundException("ShipmentType '"+id+"' Not Exist"));
	}
	...
	public boolean isShipmentTypeCodeExist(String code) 
	{
		...
		return isExist;*/         						//one way(here if false,error message will shown)
		
		//return repo.getShipmentTypeCodeCount(code) > 0 ? true: false;  //2nd way(here if false,error message will shown)
		
		return repo.getShipmentTypeCodeCount(code) > 0 ;			//3rd way(here error message will not shown)
	}
