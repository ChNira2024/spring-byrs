...
//Problem:if parent class(dependent bean) is singleton(only one object create) but child class(dependency class) is prototype(more than one object is create) then expected outcome not come..
//solution:here solution is LMI(Look-up method injection)
//Solution Look-up method injection:(define one method that return child/dependency class with no body,just return null and use @Lookup Annotation)